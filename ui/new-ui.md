...
- [~] **S2.6 FAQ panel** [PRO]: `GET {api}/faq` top questions with counts, Answer opens add-data modal prefilled, Delete row action; free tier shows chips + lock per upsell decision. (Free tier done 2026-07-09: `FaqPanel` in `next/screens/KnowledgeBase.js`, PRO chip + old intro paragraph + the old dummy questions as a faded non-interactive preview table (`.hyve-next-preview`, reusable for other pro previews) + blue upsell reusing the old title/description, UTM `faq-feature`; pro tier shows the placeholder hint until the working panel lands with P2.)
...
- [ ] **S2.8 Custom data drill-in** [PRO]: search + list (`GET {api}/knowledge`), Add data + Edit via modal, delete inside modal, limit gating. (Free-tier locked drill shipped 2026-07-09 with S2.2; this item is the working pro-owned panel, P2.)
...
- [ ] **S2.10 Website URL drill-in** [PRO]: URL validation + Crawl (`POST {api}/links`), table with Indexed/Crawling status, Update/Delete per row. (Free-tier locked drill shipped 2026-07-09 with S2.2; this item is the working pro-owned panel, P2.)
