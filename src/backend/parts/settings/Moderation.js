...
const Moderation = () => {
	...
	return (
		<div className="col-span-6 xl:col-span-4">
			<Panel header={ __( 'Moderation Settings', 'hyve-lite' ) }>
				<PanelRow>
					<p className="py-4">
						{ __(
							'With the moderation functionality, you can check whether your content is potentially harmful. Users can use it to identify content that might be harmful and take action.',
							'hyve-lite'
						) }
					</p>
					...
					{ Object.keys( moderationLabels ).map( ( moderation ) => (
						<RangeControl
							...
							initialPosition={
								settings?.moderation_threshold?.[ moderation ] /
									100 || 0.5
							}
							max={ 1 }
							min={ 0 }
							...
							allowReset
							resetFallbackValue={
								moderationLabels[ moderation ].default
							}
							className="py-4"
							onChange={ ( newValue ) =>
								onChangeModeration( moderation, newValue )
							}
						/>
					) ) }
				</PanelRow>
